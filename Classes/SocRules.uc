...
function bool TestInfectedItem(ScrnPlayerInfo SPI, PlayerController OwnerPC) {
    ...
    if ( OwnerSPI == none )
        return false;
    OwnerVirus = VirusInfo(OwnerSPI.CustomInfo(class'VirusInfo', true));
    if (OwnerVirus == none || !OwnerVirus.HasSymptoms())
        return false;

    if ( Level.TimeSeconds > NextMsgTime ) {
        SPI.PlayerOwner.ClientMessage(class'ScrnBalance'.static.ColorString(strInfectedItems,192,128,1));
