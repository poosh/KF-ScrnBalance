class ScrnWaveHandler extends Info
    dependson(ScrnTypes)
    dependson(ScrnWaveInfo);

var ScrnGameLength GL;
var class<ScrnGameLength> GameLengthClass;
var class<ScrnUserGameLength> UserGameLengthClass;
...
{
    local ScrnUserGameLength UserGL;

    super.PostBeginPlay();

    Game = ScrnGameType(Level.Game);
    if (Game.ScrnBalanceMut.bUserGames && Game.KFGameLength >= 100 && Game.KFGameLength < 200) {
        UserGL = new(none, string(Game.KFGameLength)) UserGameLengthClass;
        if (UserGL.bUserWaves) {
        ...
        GL = new(none, string(Game.KFGameLength)) GameLengthClass;
    }
}
...
function LoadGame(ScrnGameType MyGame)
{
    ...
    MapZeds[0] = GL.SmallMapZeds;
    MapZeds[1] = GL.NormalMapZeds;
    MapZeds[2] = GL.BigMapZeds;
    MapZeds[3] = GL.WideOpenMapZeds;
    ...
    if (GL.FriendlyFireScale != 0) {
        Game.ScrnBalanceMut.SetFriendlyFire(GL.FriendlyFireScale);
    }

    GL.FtgSpawnRateMod = fclamp(GL.FtgSpawnRateMod, 0.2, 1.0);
    GL.MinDifficulty = clamp(GL.MinDifficulty, Game.DIFF_MIN, Game.DIFF_MAX);
    if (GL.MaxDifficulty == 0) {
        GL.MaxDifficulty = Game.DIFF_MAX;
    }
    else {
        GL.MaxDifficulty = clamp(GL.MaxDifficulty, GL.MinDifficulty, Game.DIFF_MAX);
    }
    HardcoreDifficulty = Mut.GetHardcoreDifficulty();
    if (HardcoreDifficulty < GL.MinDifficulty) {
        Mut.SetGameDifficulty(GL.MinDifficulty);
        HardcoreDifficulty = Mut.GetHardcoreDifficulty();
    }
    else if (HardcoreDifficulty > GL.MaxDifficulty) {
        Mut.SetGameDifficulty(GL.MaxDifficulty);
        HardcoreDifficulty = Mut.GetHardcoreDifficulty();
    }
    else {
    ...
    for (i = 0; i < GL.ServerPackages.length; ++i) {
        Log("Add server package: " $ GL.ServerPackages[i], class.name);
        Game.AddToPackageMap(GL.ServerPackages[i]);
    }

    for (i = 0; i < GL.Mutators.length; ++i) {
        if (GL.Mutators[i] != "")  {
            Log("Loading additional mutator: " $ GL.Mutators[i], class.name);
            Game.AddMutator(GL.Mutators[i], true);
        }
    }

    if (GL.TraderSpeedBoost != 0) {
        Mut.bTraderSpeedBoost = GL.TraderSpeedBoost > 0;
    ...
    if (GL.bAllowZedEvents) {
        if (GL.ForceZedEvent > 0) {
            Mut.CurrentEventNum = GL.ForceZedEvent;
        }
        ...
        else if (GL.FallbackZedEvent > 0) {
            Mut.CurrentEventNum = GL.FallbackZedEvent;
    ...
    ZedInfos.length = GL.Zeds.length;
    for (i = 0; i < GL.Zeds.length; ++i) {
        zi = new(none, GL.Zeds[i]) ZedInfoClass;

        if (GL.bAllowZedEvents && zi.EventNum != Mut.CurrentEventNum
    ...
    if (GL.ZedTimeTrigger != ZT_Default) {
        Game.ZedTimeTrigger = GL.ZedTimeTrigger;
        Game.ZedTimeChanceMult = fmax(GL.ZedTimeChanceMult, 0.0);
        Game.ZedTimeDuration = clamp(GL.ZedTimeDuration, 3, 240);
    }

    Game.StartingCash = 0;  // the game must call CalcStartingCash() instead
    if (GL.StartDoshMax <= 0) {
        GL.StartDoshMax = 1000000;
    }
    else {
        RecommendedStartDosh = GL.StartDoshMax;
    }

    // this makes sure the Wave is never none
    if (GL.Waves.length == 0) {
        ...
        if (GL.StartDosh == 0 && GL.StartDoshPerWave == 0) {
            GL.StartDoshPerWave = RecommendedStartDosh / GL.Waves.length / 25 * 25;
            GL.StartDosh = GL.StartDoshPerWave;
            log("Calculated StartDoshPerWave=" $ GL.StartDoshPerWave, class.name);
        }
    }
    ...
    NextWaveNum = 0;
    Game.FinalWave = GL.Waves.length;
...
function PrecacheWaves()
{
    ...
    WaveCache.Length = GL.Waves.Length;
    for (i = 0; i < GL.Waves.Length; ++i) {
        WaveCache[i] = CreateWave(GL.Waves[i]);
    }
}
...
function bool ApplyGameDifficulty(byte HardcoreDifficulty)
{
    ...
    if (HardcoreDifficulty < GL.MinDifficulty || HardcoreDifficulty > GL.MaxDifficulty) {
        log("Game difficulty " $ HardcoreDifficulty $ " is out of founds ["$GL.MinDifficulty$".." $ GL.MaxDifficulty $ "]",
                class.name);
        return false;
    }

    if (GL.HardcoreLevel.length > 0) {
        for (i = 0; i < GL.HardcoreLevel.length; ++i) {
            if (HardcoreDifficulty >= GL.HardcoreLevel[i].Difficulty && ForceHL < GL.HardcoreLevel[i].HL)
                ForceHL = GL.HardcoreLevel[i].HL;
        }
    }
    ...
    if ( ForceHL > 0 ) {
        ...
        Mut.GameRules.ScaleHardcoreLevel(GL.HLMult);
    }
    return true;
}
...
{
    if (GL.MinBonusLevel != 255) {
        MinLevel = GL.MinBonusLevel;
    }
    if (GL.MaxBonusLevel != 255) {
        MaxLevel = GL.MaxBonusLevel;
    }
}

function bool VersionCheck()
{
    return GL.GameVersion == Wave.GameVersion || Wave.GameVersion == 0;
}
...
protected function bool LoadNextWave()
{
    ...
    if ( NextWaveNum == 0 ) {
        Game.AddSuicideTime(GL.SuicideTime * (1.0 + GL.SuicideTimePerPlayerMult * (NumPlayers - 1)), true);
    }
    ...
    else if (GL.SuicideTimePerWave != 0 && NextWaveNum != 0) {
        Game.AddSuicideTime(GL.SuicideTimePerWave * (1.0 + GL.SuicideTimePerPlayerMult * (NumPlayers - 1)), false);
    }
    ...
    switch (Wave.EndRule) {
        case RULE_ReachTrader:
            Rule = Spawn(class'ScrnWaveRule_ReachTrader');
            break;
    }
...
function string PickWaveName(array<string> Candidates) {
    ...
    while (Candidates.Length > 0) {
        ...
        if (bOk && GL.bUniqueWaves && Candidates.Length > 1 && class'ScrnF'.static.SearchStr(UsedWaves, NewWave) != -1) {
            // not unique
            bOk = false;
        }
...
function RunWave()
{
    ...
    if (GL.Doom3DisableSuperMonstersFromWave > 1 && Game.WaveNum + 1 == GL.Doom3DisableSuperMonstersFromWave) {
...
function AdjustNextSpawnTime(out float NextSpawnTime, float MinSpawnTime)
{
    ...
    if (Game.HasEnoughZeds()) {
        ...
        if (FTG != none && (FTG.TeamBases[0].bHeld || FTG.TeamBases[1].bHeld)) {
            SpawnRateMod *= GL.FtgSpawnRateMod;
        }
...
{
    return clamp(GL.StartDosh + Game.WaveNum * GL.StartDoshPerWave, GL.StartDoshMin, GL.StartDoshMax);
}
...
function LoadWeaponLists()
{
    ...
    for (i = 0; i < GL.AllowWeaponLists.Length; ++i) {
        LoadWeaponList(GL.AllowWeaponLists[i], LoadedAllowedLists, AllowWeapons);
    }
    for (i = 0; i < GL.BlockWeaponLists.Length; ++i) {
        LoadWeaponList(GL.BlockWeaponLists[i], LoadedBlockedLists, BlockWeapons);
    }
...
function bool IsItemAllowed(class<Pickup> PC)
{
    ...
    return PC != none
            && (GL.AllowWeaponPackages.Length == 0 || f.static.SearchName(GL.AllowWeaponPackages, PC.outer.name) != -1)
            && (GL.BlockWeaponPackages.Length == 0 || f.static.SearchName(GL.BlockWeaponPackages, PC.outer.name) == -1)
...
function bool IsPerkAllowed(class<ScrnVeterancyTypes> Perk)
{
    ...
    f = class'ScrnFunctions';
    return Perk != none
            && (GL.AllowPerks.Length == 0 || f.static.SearchName(GL.AllowPerks, Perk.name) != -1)
            && (GL.BlockPerks.Length == 0 || f.static.SearchName(GL.BlockPerks, Perk.name) == -1);
}
...
function FtgStinkyPickedBase()
{
    if (Game.HasEnoughZeds()) {
        Game.NextMonsterTime += GL.FtgSpawnDelayOnPickup;
    }
    LoadDialogues(Wave.FtgDialogues);
}
...
function bool LoadDialogues(out array<string> Dialogues)
{
    ...
    if (DialogueHandler == none) {
        DialogueHandler = Spawn(DialogueHandlerClass, self);
        if (DialogueHandler == none)
            return false;
...
defaultproperties
{
    ...
    UserGameLengthClass=class'ScrnUserGameLength'
